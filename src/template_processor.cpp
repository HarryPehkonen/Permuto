...
nlohmann::json TemplateProcessor::process_string(const std::string& str,
    ...
    std::string result = parser_.replace_placeholders(
        str, [this, &context](const std::string& path) -> std::string {
    ...
    for (auto it = obj.begin(); it != obj.end(); ++it) {
        ...
        if (value.is_string()) {
            ...
            if (placeholder_path) {
                ...
                if (!resolved_value
                    && options_.missing_key_behavior == MissingKeyBehavior::Remove) {
