...
nlohmann::json apply(const nlohmann::json& template_json, const nlohmann::json& context,
                     const Options& options) {
    // Validate root-level Remove mode
    if (options.missing_key_behavior == MissingKeyBehavior::Remove && template_json.is_string()) {
