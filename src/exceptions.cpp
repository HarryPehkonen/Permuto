...
PermutoException::PermutoException(const std::string& message) : std::runtime_error(message) {}
...
const std::vector<std::string>& CycleException::cycle_path() const { return cycle_path_; }
...
const std::string& MissingKeyException::key_path() const { return key_path_; }
...
size_t RecursionLimitException::depth() const { return depth_; }
} // namespace permuto