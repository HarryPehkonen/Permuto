// clavis/include/clavis/clavis.hpp
#ifndef CLAVIS_CLAVIS_HPP
#define CLAVIS_CLAVIS_HPP
...
// Forward declaration
class ClavisException; // Include exceptions.hpp if needed here, or rely on user include
...
enum class MissingKeyBehavior {
    ...
    Error   ///< Throw a ClavisMissingKeyException.
};

/**
 * @brief Configuration options for the Clavis processing engine.
 */
struct Options {
 ...
 * @throws ClavisCycleException If a cyclical dependency is detected.
 * @throws ClavisMissingKeyException If a key/path is not found and options.onMissingKey is Error.
 * @throws ClavisParseException If the input template_json has issues (though basic validity is often checked beforehand).
